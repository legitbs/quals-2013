...
void build_state_array_from_string(uchar in[], uchar state[4][4]) {
   state[0][0] = in[0];
   state[1][0] = in[1];
   state[2][0] = in[2];
   state[3][0] = in[3];
...
void build_string_from_state_array(uchar state[4][4], uchar out[]) {
   out[0] = state[0][0];
   out[1] = state[1][0];
   out[2] = state[2][0];
   out[3] = state[3][0];
...
void aeslite_encrypt(uchar in[], uchar out[], uint key[], int keysize)
{
   uchar state[4][4];
...
void aeslite_decrypt(uchar in[], uchar out[], uint key[], int keysize)
{
   uchar state[4][4];
   // Copy the input to the state.
   build_state_array_from_string(in, state);

   InvMixColumns(state); InvShiftRows(state); InvSubBytes(state);
   AddRoundKey(state,&key[0]);
